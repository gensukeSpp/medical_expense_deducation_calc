...
実行手順 (エージェントが実行する内容):
1. ベースブランチと現在ブランチの差分を取得する（例: git fetch origin && git diff --name-only origin/main...HEAD）。また、指定された PR ID がある場合は `gh pr view {ID}` で PR の head ブランチ情報を取得し、現ブランチと比較する（例: git fetch origin && git diff --name-only origin/{pr_head}...HEAD）。
2. 差分にソースコード変更が含まれていれば、スナップショットファイルを生成して docs/architecture/ に追加する。ファイル名は YYYY-MM-DD-{num}-architecture.md の形式とし、変更されたコミット摘要と変更ファイル一覧を含めるテンプレートを使用する。
...
Use when:
- コード変更でアーキテクチャに影響があるとき（設計書のスナップショットを残したい場合）。
- 大規模リファクタやモジュール分割を行い、設計差分をドキュメント化してレビュープロセスに含めたいとき。
- アーキテクチャ図や依存関係の更新を行い、目次や一覧を自動で更新したいとき。
- リリースノートや技術的負債の記録としてスナップショットを作成したいとき。
- 他チームが作成した PR と自分の作業ブランチを比較して合わせ込みを確認したいとき。
